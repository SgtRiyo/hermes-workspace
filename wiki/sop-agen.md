...
2. Lead (`kr/glm-5`): merencanakan, me-manage sub-agent, troubleshooting.
3. Sub-agent (`kr/deepseek-3.2`, hemat): tugas rutin — rekap, baca teks, draft.
4. Fallback otomatis: `kr/glm-5` → `kr/deepseek-3.2` → `kr/qwen3-coder-next` (config `~/.hermes/config.yaml` → `fallback_providers`).
